...
let str3 = "asd asdfgh asdfghjkl; qwertyuiop[]zxc";

function getLongestInt (str) {
  let compear = function (arr) {
     let x = arr.join("").split(" ").join("");
     return x.length;
  }
  let result = [];
  let arr = str.split("");
  ...
  let x = ["x"];
  for (let i = 0; i < arr.length; i++) {
        if (compear(newArr) > compear(result)) {
        result = newArr;
      }
    if (newArr.includes(arr[i]) && arr[i] !== " ") {
      arr.splice(0, newArr.indexOf(arr[i]) + 1);
...
console.log(getLongestInt(str3));
Ex.6.
let str1 = "dfgjkloyp";
let str2 = "aweyoolp";
